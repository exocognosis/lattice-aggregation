...
const PROOF_DOC_ANCHORS: &[(&str, &[&str])] = &[
    ...
    (
        ...
        &[
            ...
            "lattice-estimator NOT run",
            "not a closed security claim",
        ],
    ),
    (
        "docs/cryptography/design-space-boundary-theorems.md",
        &[
            ...
            "## DSB-3. Theorem FST-T4",
            "## DSB-4. Theorem FST-T5",
            "exact threshold ML-DSA-65 *exists*",
            "not yet a formal impossibility lemma",
        ],
    ),
    (
        "docs/cryptography/distributed-mask-mpc-feasibility.md",
    ...
    (
        "docs/cryptography/fst-l12-committee-cost-model.md",
        &[
            "# FST-L12: Committee-MPC Cost Model",
            "## L12-0. Purpose",
            "## L12-5. Go / no-go against real cadences",
            "epoch certificate, not per block",
            "scripts/model_fst_l12_committee_cost.py",
        ],
    ),
    (
        "docs/cryptography/hypothesis-outcome-taxonomy.md",
    ...
    (
        "docs/cryptography/partial-soundness-advancement-2026-07-12.md",
        &[
            "# Criterion 4 Advancement",
            "## What Evidence Now Exists",
            "## What Criterion 4 Still Requires for Closure",
            "verify_module_partial_local_validity",
    ...
    (
        ...
        &[
            ...
            "FST-T7 two-stack boundary",
            "FST-T8 design-space boundary",
            "FST-L12 committee-MPC feasibility",
...
fn release_readiness_checklist_keeps_required_anchor_contract() {
    assert_contains_all(
        ...
        &[
            ...
            "requires security evidence review",
...
fn benchmark_docs_keep_simulation_and_real_world_boundaries() {
    assert_contains_all(
        ...
        &[
            ...
            "deterministic research telemetry",
            "requires security evidence review",
            "requires real-world validator performance evidence",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "minimum evidence needed for real-world benchmark results",
            "FIPS validation",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "requires security evidence review",
            "requires real-world validator performance evidence",
            "requires production-readiness evidence",
...
fn related_work_comparator_docs_keep_claim_boundary() {
    assert_contains_all(
        ...
        &[
            ...
            "publication boundary is therefore comparative",
        ],
    );
    assert_contains_all(
        RELEASE_READINESS_CHECKLIST,
...
fn production_coordinator_docs_keep_claim_boundary() {
    assert_contains_all(
        ...
        &[
            ...
            "Aggregate standard-verifier compatibility requires",
            "real threshold recomputation, full KAT, bridge-test, proof, and audit gates",
            "EpsilonLedger",
            "blinded pre-filter",
            "Renyi divergence",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "production threshold security evidence requirements",
            "aggregate threshold verification",
            "`src/adapter/production_wire.rs`",
            "Gated hazmat/conformance track",
        ],
    );
    assert_contains_all(
...
fn readme_tracks_current_research_preview_overview() {
    assert_contains_all(
        ...
        &[
            ...
            "remaining cryptographic backend, proof, validation, and audit artifacts",
...
fn production_acceptance_docs_keep_claim_boundary() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            "`LocalAccept`",
            "`AggregateAccept`",
            "hazmat/conformance-track typed acceptance predicates",
            "production partial verification",
...
fn blocker_evidence_docs_keep_claim_boundary() {
    assert_contains_all(
        ...
        &[
            ...
            "conformance/proof-review gates",
            "Production threshold ML-DSA security requires selected-backend evidence",
            "CAVP/ACVTS validation requires validation artifacts",
            "FIPS validation requires validation artifacts",
            "rejection-distribution preservation requires proof artifacts",
            "Selected-backend proof closure requires checked proof artifacts",
            "standard-verifier compatibility requires checked proof artifacts",
            "stricter release-gate coverage",
            "Criterion promotion requires selected-backend artifacts",
            "hazmat conformance track",
            "completed Renyi proof requires selected-backend evidence",
            "completed standard-verifier compatibility requires checked proof artifacts",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "Evidence gates, sample-vector provider conformance, fixture-backed bridge conformance evidence, selected-backend aggregate-output artifact gate coverage, selected-backend threshold-output artifact gate coverage, selected-backend proof-closure artifact package gate coverage, and closure frameworks feed",
            ...
            "selected-backend aggregate recomputation requires checked proof artifacts",
        ],
    );
    assert_contains_all(
        "docs/cryptography/protocol-code-crosswalk.md",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "full KAT/validation artifact slots",
            "theorem-linkage artifact digest",
            "strict backend emission capture",
    ...
    assert_contains_all(
        ...
        &[
            ...
            "reviewed source package digest",
            "ordinary single-key standard-provider output",
            "strict external backend capture is now",
            "conformance/proof-review evidence",
            "requires selected-backend proof closure evidence",
            "requires production threshold ML-DSA security evidence",
            "requires CAVP/ACVTS validation evidence",
...
fn cryptography_readme_indexes_current_proof_docs() {
    assert_contains_all(
        ...
        &[
            ...
            "bdlop-parameter-security-estimate.md",
            "design-space-boundary-theorems.md",
            "distributed-mask-mpc-feasibility.md",
            "epsilon-mask-fork-decision.md",
            "epsilon-mask-fork-reconciliation.md",
            "formal-security-theorem.md",
            "formal-threshold-mldsa-transcript.md",
            "fst-l12-committee-cost-model.md",
            "hypothesis-outcome-taxonomy.md",
            "ideal-functionality.md",
            "mask-distribution-evidence.md",
            "noise-rejection-proof-plan.md",
            "partial-soundness-advancement-2026-07-12.md",
            ...
            "threshold-stack-architecture.md",
...
fn protocol_code_crosswalk_maps_protocol_phases_to_code_and_tests() {
    assert_contains_all(
        ...
        &[
            ...
            "navigation aid for implementation",
            "Selected-backend artifacts for real ML-DSA",
            "Distributed Stack A Research Cryptography",
            "Coordinator Stack B Verifier-Compatible Path",
            "Epsilon-Mask MPC Feasibility",
            "`src/protocol.rs`",
