...
external/pure, empty-context vector `tcId = 35` from
`usnistgov/ACVP-Server/gen-val/json-files/ML-DSA-sigVer-FIPS204/internalProjection.json`.
The current gate skips non-empty context, prehash, internal, and external-mu
vectors because the public hazmat verifier API does not yet expose those entry
points.
