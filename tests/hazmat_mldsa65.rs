...
use dytallix_pq_threshold::{
    low_level::mldsa65::{
        check_poly_bound, compute_verification_w1, expand_a, fips_inverse_ntt, fips_ntt,
        fips_zeta_table_mont, inverse_ntt, matrix_vector_mul, ntt, pack_public_key, pack_signature,
        poly_add, poly_mul_ntt, poly_negacyclic_mul, poly_shift_left_d, poly_sub, reduce_mod_q,
        rej_ntt_poly, sample_in_ball, t1_times_2d, unpack_public_key, unpack_signature,
        use_hint_vector, verify_mldsa65_external_pure, verify_mldsa65_internal_mu,
...
#[test]
fn hazmat_fips_zeta_table_matches_reference_prefix() {
    assert_eq!(
        &fips_zeta_table_mont()[..16],
        &[
            4_193_792, 25_847, 5_771_523, 7_861_508, 237_124, 7_602_457, 7_504_169, 466_468,
            1_826_347, 2_353_451, 8_021_166, 6_288_512, 3_119_733, 5_495_562, 3_111_497, 2_680_103,
        ]
    );
}

#[test]
fn hazmat_fips_ntt_matches_reference_vector_slices() {
    let transformed = fips_ntt(&fips_ntt_fixture_poly());

    assert_eq!(
        &transformed.coeffs[..16],
        &[
            9_442_680, 2_619_710, 959_145, -300_079, -5_401_690, -8_369_568, -149_485, 756_535,
            6_686_234, 1_187_872, -664_982, -2_431_840, 10_759_656, 7_918_966, 200_949, 3_545_097,
        ]
    );
    assert_eq!(
        &transformed.coeffs[240..],
        &[
            4_791_430, 2_930_798, -2_904_820, -2_793_432, 2_192_520, 127_546, 2_480_410, 9_718_924,
            8_649_941, 1_420_297, 1_644_933, 1_452_653, -2_252_320, -5_623_196, -2_015_584,
            -115_172,
        ]
    );
}
...
#[test]
fn hazmat_fips_inverse_ntt_round_trips_reference_vector() {
    let input = fips_ntt_fixture_poly();

    assert_eq!(fips_inverse_ntt(&fips_ntt(&input)), input);
}
...
fn fips_ntt_fixture_poly() -> Poly {
    let mut poly = Poly::zero();
    for (index, coeff) in poly.coeffs.iter_mut().enumerate() {
        *coeff = ((index as i32 * 29) + 700).rem_euclid(Q);
    }
    poly
}
