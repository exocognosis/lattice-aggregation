...
| Post-119 distributed cryptographic substrate | Real distributed-secret research components must be tracked separately from the standard-verifier coordinator track so no evidence package conflates no-single-holder with verifier-compatible output. | `src/crypto/vss_real.rs`, `src/crypto/bdlop.rs`, `src/crypto/bdlop_pok.rs`, `src/crypto/vss_bdlop.rs`, `src/crypto/mldsa_module.rs`, `src/crypto/mldsa_dkg.rs`, `src/crypto/distributed_nonce.rs`, `src/crypto/share_transport.rs`, `src/crypto/mldsa_primitives.rs`, `src/low_level/ntt.rs`, `src/backend/module_partial.rs` | library tests under those modules, `tests/partial_soundness_real_local_verifier.rs`, `tests/threshold_core.rs` | Real VSS/DKG, BDLOP PoK, distributed nonce, ML-DSA module arithmetic, and local partial-validity checks are now executable evidence. They remain research/proof-review evidence: Stack A does not emit FIPS verifier-valid signatures, Stack B still relies on a coordinator no-export assumption, and `epsilon_mask` remains open. |
