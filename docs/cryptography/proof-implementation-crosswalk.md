...
| Contribution backend instantiation route | A selected production backend must declare its relation, statement schema, soundness theorem, hiding theorem, extractor or substitute, leakage function, and audit status before `eps_contrib` can be closed. | `src/crypto/contribution_proof.rs`, `src/crypto/production_policy.rs`, `docs/cryptography/contribution-backend-instantiation.md` | `tests/contribution_proof.rs`, `tests/production_policy.rs`, `tests/proof_documentation_manifest.rs` | Route documented; no concrete zero-knowledge, MPC, interactive, or ideal contribution backend is instantiated. |
| Unauthorized output classifier closure route | The S7 -> S8 reduction must classify every accepting unauthorized aggregate output into exactly one ML-DSA forgery or threshold-side assumption violation, with `eps_cls_unmapped = 0`. | `docs/cryptography/simulator-hybrid-reductions.md`, `docs/cryptography/unauthorized-output-classifier-closure.md` | `tests/proof_documentation_manifest.rs` | Route documented; production grammar, reductions, and zero-unmapped proof remain open. |
