...
| `eps_rej` | Mismatch between threshold aggregate rejection and centralized ML-DSA-65 rejection on the same candidate values. | Proof route documented; predicate proof open. | [eps-rej-predicate-sublemmas.md](eps-rej-predicate-sublemmas.md), [eps-rej-theorem-closure.md](eps-rej-theorem-closure.md), [rejection-sampling-closure-plan.md](rejection-sampling-closure-plan.md), [rejection-sampling-theorem-closure.md](rejection-sampling-theorem-closure.md), [rejection-predicate-equivalence.md](rejection-predicate-equivalence.md), [rejection-sampling-bounds.md](rejection-sampling-bounds.md), hazmat rejection tests. | Prove equivalence of `z`, low-bit, `ct0`, hint, challenge, active-set, encoding, strictness, malformed-input, and verifier-side high-bit predicates, or charge every mismatch to an explicit subterm. | Predicate maps and boundary tests exist. They do not prove accepted threshold signatures match centralized ML-DSA rejection. |
| `eps_withhold` | Selective-abort, retry, timeout, post-commitment withholding, post-challenge withholding, and observable abort-label bias. | Proof route documented; open proof obligation and operational blocker. | [eps-withhold-simulator-obligations.md](eps-withhold-simulator-obligations.md), [eps-withhold-theorem-closure.md](eps-withhold-theorem-closure.md), [fst-l7-abort-compatibility.md](fst-l7-abort-compatibility.md), [withholding-abort-bound.md](withholding-abort-bound.md), [active-adversary-model.md](active-adversary-model.md), [rejection-sampling-theorem-closure.md](rejection-sampling-theorem-closure.md), [rejection-sampling-bounds.md](rejection-sampling-bounds.md), actor simulation tests. | Fix retry limits, timeout/exclusion policy, acceptance-probability lower bounds, simulator behavior, observable abort transcript `O_abort`, and evidence/release noninterference. | The repository decomposes selective-abort obligations. It does not claim bounded selective-abort advantage or production liveness. |
| `eps_contrib` | Loss from proving that every accepted contribution is context-bound, relation-valid, simulatable or extractable as required, and witness hiding under the selected leakage model. | Idealized route for immediate theorem; production backend not selected. | [f-contrib-realization-simulator.md](f-contrib-realization-simulator.md), [f-contrib-ideal-functionality.md](f-contrib-ideal-functionality.md), [eps-contrib-backend-decision-record.md](eps-contrib-backend-decision-record.md), [eps-contrib-backend-proof-route.md](eps-contrib-backend-proof-route.md), [contribution-backend-decision-record.md](contribution-backend-decision-record.md), [fst-l4-partial-share-validity.md](fst-l4-partial-share-validity.md), [contribution-backend-selection.md](contribution-backend-selection.md), [contribution-soundness-relation.md](contribution-soundness-relation.md), [contribution-backend-instantiation.md](contribution-backend-instantiation.md), `src/crypto/contribution_proof.rs`. | Use `F_CONTRIB` only as an IdealVSS proof-isolation device, then replace it with a proof, MPC, interactive, or ideal-realization theorem before production claims. | The repository defines the contribution-backend replacement route and immediate idealization decision. Current transcript-hash payloads are not production contribution proofs. |
| `eps_classify` | Residual probability that an unauthorized accepting aggregate output is not mapped to a base ML-DSA forgery or named threshold-side assumption violation. | Proof route documented; must be eliminated for final theorem. | [eps-classify-unmapped-zero-theorem.md](eps-classify-unmapped-zero-theorem.md), [eps-classify-totality-disjointness-closure.md](eps-classify-totality-disjointness-closure.md), [eps-classify-per-case-reductions.md](eps-classify-per-case-reductions.md), [eps-classify-elimination-route.md](eps-classify-elimination-route.md), [fst-l10-classifier-closure.md](fst-l10-classifier-closure.md), [fst-l6-no-subthreshold-signing.md](fst-l6-no-subthreshold-signing.md), [unauthorized-output-classifier-elimination.md](unauthorized-output-classifier-elimination.md), [unauthorized-output-classifier-closure.md](unauthorized-output-classifier-closure.md), [simulator-hybrid-reductions.md](simulator-hybrid-reductions.md). | Define the production verifier grammar, prove classifier totality and disjointness, supply per-case reductions, and prove `eps_cls_unmapped = 0`. | The repository decomposes unauthorized-output classification. It does not prove final unforgeability. |
| `eps_verify` | Residual standard-verifier compatibility loss for proving accepted threshold outputs verify as unmodified ML-DSA-65 signatures. | Implemented engineering evidence plus open proof decision. | [eps-verify-to-rej-absorption-theorem.md](eps-verify-to-rej-absorption-theorem.md), [eps-verify-rejection-absorption-closure.md](eps-verify-rejection-absorption-closure.md), [eps-verify-absorption-decision-record.md](eps-verify-absorption-decision-record.md), [eps-verify-absorption-decision.md](eps-verify-absorption-decision.md), [fst-l5-aggregation-correctness.md](fst-l5-aggregation-correctness.md), [correctness-lemmas.md](correctness-lemmas.md), [rejection-predicate-equivalence.md](rejection-predicate-equivalence.md), hazmat standard-verifying tests. | Decide whether this term is folded into `eps_rej` or remains separate; then prove byte-level signature layout, high-bit reconstruction, hint use, and verification acceptance for all accepted threshold transcripts. | Hazmat paths exercise standard-size verification. This is not a complete compatibility theorem. |
...
The dedicated epsilon subterm closure routes are
[eps-mask-theorem-closure.md](eps-mask-theorem-closure.md),
[eps-rej-theorem-closure.md](eps-rej-theorem-closure.md), and
[eps-withhold-theorem-closure.md](eps-withhold-theorem-closure.md). They expand
the mask, predicate, and withholding obligations for review traceability; they
do not close or numerically bound those residuals.

The Residual Closure Batch A route refinements are
...
They make the game, predicate, and simulator obligations more explicit without
claiming any residual is zero, negligible, or production-ready.

Residual Closure Batch B refines contribution, verifier-compatibility, and
classifier terms through
...
documents are roadmap artifacts only; they do not select a contribution
backend, decide verifier absorption, or prove `eps_cls_unmapped = 0`.

Residual Closure Batch C narrows the next decisions through
...
[eps-vss-production-route.md](eps-vss-production-route.md). These files
recommend immediate ideal contribution handling, conservative verifier carry,
per-case classifier obligations, and VSS/DKG production requirements without
closing any residual.

Residual Closure Batch D turns those decisions into theorem-closure interfaces:
...
These files clarify ideal functionality, byte-level verifier absorption,
classifier totality/disjointness, and production DKG obligation boundaries
without proving or closing the residuals.

Residual Closure Batch E drafts the first formal-reduction shapes:
...
These drafts define simulator hybrids, verifier absorption hybrids, the
...
8. Close [rejection-sampling terms](rejection-sampling-closure-plan.md),
   including mask distribution and rejection predicate equivalence, through the
   [rejection-sampling theorem closure batch](rejection-sampling-theorem-closure.md)
   and the dedicated [eps_mask](eps-mask-theorem-closure.md) and
   [eps_rej](eps-rej-theorem-closure.md) closure routes.
9. Close withholding, retry, release, and evidence noninterference through the
   rejection-sampling closure plan and the
   [eps_withhold closure route](eps-withhold-theorem-closure.md).
10. Prove verifier compatibility and decide the final treatment of
   `eps_verify`, using the
   [eps_verify absorption decision route](eps-verify-absorption-decision.md).
11. Eliminate `eps_classify` through the
   [classifier elimination plan](unauthorized-output-classifier-elimination.md)
   and [eps_classify elimination route](eps-classify-elimination-route.md).
