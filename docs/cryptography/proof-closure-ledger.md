...
| `eps_contrib` | Loss from proving that every accepted contribution is context-bound, relation-valid, simulatable or extractable as required, and witness hiding under the selected leakage model. | Idealized route for immediate theorem; production backend not selected. | [eps-contrib-backend-proof-route.md](eps-contrib-backend-proof-route.md), [contribution-backend-decision-record.md](contribution-backend-decision-record.md), [fst-l4-partial-share-validity.md](fst-l4-partial-share-validity.md), [contribution-backend-selection.md](contribution-backend-selection.md), [contribution-soundness-relation.md](contribution-soundness-relation.md), [contribution-backend-instantiation.md](contribution-backend-instantiation.md), `src/crypto/contribution_proof.rs`. | Use `F_CONTRIB` only as an IdealVSS proof-isolation device, then replace it with a proof, MPC, interactive, or ideal-realization theorem before production claims. | The repository defines the contribution-backend replacement route and immediate idealization decision. Current transcript-hash payloads are not production contribution proofs. |
| `eps_classify` | Residual probability that an unauthorized accepting aggregate output is not mapped to a base ML-DSA forgery or named threshold-side assumption violation. | Proof route documented; must be eliminated for final theorem. | [eps-classify-elimination-route.md](eps-classify-elimination-route.md), [fst-l10-classifier-closure.md](fst-l10-classifier-closure.md), [fst-l6-no-subthreshold-signing.md](fst-l6-no-subthreshold-signing.md), [unauthorized-output-classifier-elimination.md](unauthorized-output-classifier-elimination.md), [unauthorized-output-classifier-closure.md](unauthorized-output-classifier-closure.md), [simulator-hybrid-reductions.md](simulator-hybrid-reductions.md). | Define the production verifier grammar, prove classifier totality and disjointness, supply per-case reductions, and prove `eps_cls_unmapped = 0`. | The repository decomposes unauthorized-output classification. It does not prove final unforgeability. |
| `eps_verify` | Residual standard-verifier compatibility loss for proving accepted threshold outputs verify as unmodified ML-DSA-65 signatures. | Implemented engineering evidence plus open proof decision. | [eps-verify-absorption-decision.md](eps-verify-absorption-decision.md), [fst-l5-aggregation-correctness.md](fst-l5-aggregation-correctness.md), [correctness-lemmas.md](correctness-lemmas.md), [rejection-predicate-equivalence.md](rejection-predicate-equivalence.md), hazmat standard-verifying tests. | Decide whether this term is folded into `eps_rej` or remains separate; then prove byte-level signature layout, high-bit reconstruction, hint use, and verification acceptance for all accepted threshold transcripts. | Hazmat paths exercise standard-size verification. This is not a complete compatibility theorem. |
