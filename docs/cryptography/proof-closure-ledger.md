...
| `eps_ro` | Random-oracle programming, typed-domain separation, prior-query, and byte-encoding injectivity losses. | Proof route documented; byte-level proof open. | [production-transcript-grammar.md](production-transcript-grammar.md), [random-oracle-commitment-closure.md](random-oracle-commitment-closure.md), [random-oracle-game.md](random-oracle-game.md), [formal-threshold-mldsa-transcript.md](formal-threshold-mldsa-transcript.md), transcript determinism tests. | Prove injective encodings and domain separation for `H_mu`, `H_w`, `H_c`, `H_vss`, and `H_contrib`; quantify prior-query losses across sessions, retries, validators, and oracle queries. | The repository fixes random-oracle domains and transcript fields. It does not complete the ROM reduction. |
...
2. Select the [contribution backend](contribution-backend-selection.md) and
   close `eps_contrib`.
3. Lock the [production transcript grammar](production-transcript-grammar.md),
