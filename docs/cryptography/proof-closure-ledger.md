...
3. Select the [contribution backend](contribution-backend-selection.md) and
   close `eps_contrib`.
4. Lock the [production transcript grammar](production-transcript-grammar.md),
   then close [random-oracle and commitment](random-oracle-commitment-closure.md)
   non-adaptivity terms.
5. Close [rejection-sampling terms](rejection-sampling-closure-plan.md),
   including mask distribution and rejection predicate equivalence.
6. Close withholding, retry, release, and evidence noninterference through the
   rejection-sampling closure plan.
7. Prove verifier compatibility and decide the final treatment of
   `eps_verify`.
8. Eliminate `eps_classify` through the
   [classifier elimination plan](unauthorized-output-classifier-elimination.md).
9. Replace ideal VSS/DKG with a concrete backend theorem and independent audit.
