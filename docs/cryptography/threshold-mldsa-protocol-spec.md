...
crate now has two relevant execution paths:

- the default deterministic simulation backend used for API and actor tests
- the `hazmat-real-mldsa` backend, which drives local FIPS 204 ML-DSA-65
  internals through typed wire frames and emits standard-verifying signature
  bytes

The concrete hazmat transcript is specified in
`hazmat-real-mldsa-protocol.md`. A publishable cryptographic result still
requires the proof obligations listed in `noise-bound-proof-outline.md` and
the adversary model in `security-model.md`. The theorem scaffold and hybrid
sequence are tracked in `formal-proof-scaffold.md`.
...
The default `SimulatedBackend` does not produce real ML-DSA signatures. The
feature-gated `hazmat-real-mldsa` backend does produce standard ML-DSA-65
signature bytes for research experiments, while retaining a hazmat warning
because its current transcript exposes raw contribution terms.
