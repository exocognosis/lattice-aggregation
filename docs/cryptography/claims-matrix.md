...
| Standard ML-DSA verification compatibility | No current aggregate signature is claimed to verify as a standard ML-DSA-65 signature. | Correctness Lemma 7, FST-L5 | open | A selected-backend aggregate-output artifact gate now binds accepted aggregate conformance evidence to recomputation and bridge digests, but it is not selected-backend proof closure and not a completed standard-verifier compatibility proof. Keep compatibility language as future work until real selected-backend aggregate outputs, real threshold recomputation, reviewed proof/audit artifacts, and required KAT/validation gates exist. |
