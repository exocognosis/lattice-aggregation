...
| Standard ML-DSA verification compatibility | No current aggregate signature is claimed to verify as a standard ML-DSA-65 signature. | Correctness Lemma 7, FST-L5 | open | Keep all standard-verifier compatibility language as future work until real selected-backend aggregate outputs, real threshold recomputation, reviewed proof/audit artifacts, and required KAT/validation gates exist. The checked-in bridge fixture is conformance evidence only. |
