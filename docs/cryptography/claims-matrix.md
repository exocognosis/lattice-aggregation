...
| Standard ML-DSA verification compatibility | The real standard-provider aggregate-output package path can bind one provider-verified ML-DSA-65 candidate signature through acceptance, recomputation, and bridge digest evidence; the selected-backend threshold-output artifact gate can bind that evidence to a threshold-output source package for proof review; the selected-backend proof-closure artifact package gate can bind those outputs to full KAT/validation artifact slots, rejection-distribution review, standard-verifier compatibility artifacts, and a theorem-linkage artifact digest; and the real-threshold backend emission ingestion artifact defines the future 10,000-validator verifier tuple input path. No threshold aggregate signature is claimed to verify as a standard ML-DSA-65 signature today. | Correctness Lemma 7, FST-L5 | open | A selected-backend aggregate-output artifact gate now binds accepted aggregate conformance evidence to recomputation and bridge digests; the real standard-provider package path is stronger than fixture-only bridge confidence; the selected-backend threshold-output artifact gate is stronger than real standard-provider aggregate-output package evidence; the selected-backend proof-closure artifact package gate is stronger than the selected-backend threshold-output artifact gate; and the real-threshold backend emission ingestion artifact rejects deterministic simulation and ordinary single-key standard-provider output as closure evidence while binding backend source, implementation, and transcript digests. It is still not selected-backend proof closure and not a completed standard-verifier compatibility proof. Keep compatibility language as future work until actual real threshold backend emissions, reviewed proof/audit artifacts, full KAT/validation gates, rejection-distribution preservation, and required standard-verifier compatibility proof artifacts exist. |
