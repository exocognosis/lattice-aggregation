...
Status: traceability matrix for proof targets and required completion artifacts.
...
| --- | --- | --- | --- | --- |
| FST-T1 threshold unforgeability | `formal-security-theorem.md` | Prove threshold EUF-CMA security for static corruption of at most `t - 1` validators from FST-A1 through FST-A8 and FST-L1 through FST-L7. | open | Requires the selected-backend reduction package and backend-run evidence. |
...
| FST-L8 ideal extraction | `formal-security-theorem.md`, `ideal-functionality.md` | Extract or simulate real invalid-share, abort, and aggregate-output events into ideal functionality calls. | open | Requires simulator construction and extraction proof artifacts. |
...
| Correctness Lemma 5 transcript challenge binding | `correctness-lemmas.md`, `formal-security-theorem.md` | Prove challenge bytes are determined by exactly the listed transcript fields. | implemented engineering guard | Implemented for simulation transcript construction; full ML-DSA proof artifacts are tracked separately. |
...
| DKG output agreement and robustness | `vss-dkg-security-plan.md`, `active-adversary-model.md`, `threshold-stack-architecture.md`, `distributed-keygen-capability-boundary.md` | Prove honest validators finalize one accepted-dealer set, transcript, share relation, and public key. | proof sketch only | Exact public-key bytes and an in-process encrypted receiver-custody seam are implemented as engineering guards. The path does not jointly sample shares, bind retained `t0` signing state, enforce process-isolated custody, or supply a malicious-secure DKG proof, and it is not wired to standard-verifier signature emission. |
...
- Rows marked `implemented engineering guard` feed proof review for FST-T1,
  FST-T2, active-adversary security, and production ML-DSA compatibility.
