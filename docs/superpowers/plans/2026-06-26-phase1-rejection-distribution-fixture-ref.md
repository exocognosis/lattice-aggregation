# Phase 1 Criterion 2 Rejection-Distribution Fixture Reference Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Promote the P1 rejection-distribution review proof slot into the checked Criterion 2 `artifact_fixture_refs` contract.

**Architecture:** Add one checked JSON fixture for the existing `rejection_distribution_review_digest` typed slot, wire it into the Criterion 2 manifest and assessor constant, and update docs without changing production logic or promoting Criterion 2 beyond `partially_met`.
...
Extend `criterion2_manifest_links_checked_fixture_refs` so it requires the rejection-distribution review fixture ref and verifies the fixture file exists.

- [x] **Step 3: Add Rust typed fixture failing tests**

Add a rejection-distribution review fixture parser, typed-slot match test, and fixture package drift test. The tests should initially fail because `tests/fixtures/p1_rejection_distribution_review_artifact_fixture.json` does not exist.
...
CARGO_TARGET_DIR=/private/tmp/lattice-phase1-rejection-red-target cargo test --features coordinator-assisted --test criterion2_proof_substance_manifest criterion2_manifest_links_checked_fixture_refs
CARGO_TARGET_DIR=/private/tmp/lattice-phase1-rejection-red-target cargo test --features coordinator-assisted --test production_rejection_equivalence rejection_distribution_review_artifact_fixture
```

### Task 2: Add The Checked Fixture

**Files:**
- Add: `tests/fixtures/p1_rejection_distribution_review_artifact_fixture.json`
- Modify: `tests/production_rejection_equivalence.rs`
...
Parse the fixture, bind it to `proof_slot_artifacts.rejection_distribution_review_artifact`, check the source digest, review digest, threshold-output certificate digest, transcript binding digest, typed artifact digest, proof-closure package digest field, and negative-case names.

- [x] **Step 3: Add drift guard**

Add a deterministic package digest test using domain separator `lattice-aggregation:p1-rejection-distribution-review-artifact-fixture-package:v1`.
...
Append the rejection-distribution review fixture metadata to `CRITERION2_ARTIFACT_FIXTURE_REFS`.

- [x] **Step 2: Add manifest fixture ref and evidence ref**

Append matching metadata to `proof_payload.artifact_fixture_refs` and add the fixture path to `evidence_refs` in `criterion-2-proof-substance.json`.

- [x] **Step 3: Update synthetic report fixture**

Add the same fixture metadata and markdown anchor phrases to `write_criterion2_proof_substance_formalization`.
...
```sh
CARGO_NET_OFFLINE=true CARGO_TARGET_DIR=/private/tmp/lattice-phase1-rejection-target cargo test --features coordinator-assisted --test criterion2_proof_substance_manifest --test proof_documentation_manifest --test production_rejection_equivalence
```

- [x] **Step 3: Run assessment**

```sh
python3 scripts/assess_lattice_hypothesis.py --root . --out /private/tmp/lattice-phase1-rejection-assessment --offline --target-dir /private/tmp/lattice-phase1-rejection-assessment-target
```
