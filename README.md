...
- real-threshold backend emission gate for the 10,000-validator target; this threshold verifier closure contract rejects deterministic simulation and ordinary single-key standard-provider output as closure evidence, and remains conformance/proof-review evidence only
...
The real threshold backend emission gate is tracked in the same 10,000-validator document and in [docs/cryptography/rejection-equivalence-evidence.md](docs/cryptography/rejection-equivalence-evidence.md). It defines the future closure contract for `P1RealThresholdVerifierClosurePackage`: real threshold ML-DSA backend provenance, 10,000 validators, threshold 6,667, a 3,309-byte aggregate signature, standard-verifier acceptance, and mutation rejection. It is not ordinary single-key standard-provider output and does not promote Criterion 2 beyond `partially_met`.
