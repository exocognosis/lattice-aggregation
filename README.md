...
- canonical backend-emission capture schema/importer for future external real-threshold backend outputs; `P1RealThresholdBackendEmissionCapture` decodes `lattice-aggregation:p1-real-threshold-backend-emission-capture:v1`, rejects schema fixtures before tuple ingestion, binds predecessor certificate digests and expected package digests, and feeds `derive_p1_verified_real_threshold_backend_emission_artifact_package_from_capture`
- repo-generated backend-emission request manifest for future external real-threshold backend outputs; [scripts/build_backend_emission_request.py](scripts/build_backend_emission_request.py) writes `lattice-aggregation:p1-real-threshold-backend-emission-request:v1` as the P1 challenge contract an external backend must answer before capture, including predecessor digests, message bytes, required capture schema, mutation-rejection requirements, and forbidden localnet/simulation/fixture sources
- actual backend capture runner for future external real-threshold backend outputs; `derive_p1_verified_real_threshold_backend_emission_capture` emits capture JSON only from an artifact-ready `RealThresholdMldsa` package, and [scripts/run_backend_emission_capture.py](scripts/run_backend_emission_capture.py) rejects localnet/simulation command sources and non-importable capture shapes before artifact write, but this is `evidence_present_unclosed` conformance/proof-review evidence only and does not change the `partially_proven` / `partially_met` status
- repo-owned hazmat threshold backend capture adapter for explicit external backend checkouts; [scripts/run_hazmat_threshold_backend_capture.py](scripts/run_hazmat_threshold_backend_capture.py) requires `--backend-crate` or `LATTICE_HAZMAT_THRESHOLD_BACKEND_CRATE`, generates a temporary Rust emitter for a `dytallix-pq-threshold` hazmat backend, bridges the 10,000-validator threshold session to the standard external-message verifier boundary, records mutation-rejection evidence, and emits request-bound capture JSON for [scripts/run_backend_emission_capture.py](scripts/run_backend_emission_capture.py); this remains `evidence_present_unclosed` conformance/proof-review infrastructure only
...
- checked backend-emission capture schema fixture at [tests/fixtures/p1_real_threshold_backend_emission_capture_schema_fixture.json](tests/fixtures/p1_real_threshold_backend_emission_capture_schema_fixture.json); this pins the future real-threshold capture envelope but remains blocked until actual backend-generated threshold emission artifacts replace the schema fixture
